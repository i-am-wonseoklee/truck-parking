...
> **Note:** This vehicle model will be used for Hybrid A\* implementation without any modification.

> **Note:** For reinforcement learing, this model will be sligltly modified as described in Section III.
...
For driving direction change (i.e., gear change), the fixed time penalty will be added.
...
> **Note:** Obviously, the angle components $(\psi, \phi)$ are interpolated using the wrapped shortest-angle difference.
...
3. The observation (i.e., tokens) at step $k$ is constructed as follows:
    - `parameters`: Truck parameters are serialized into fixed-size vectors, split into meaningful chunks.
    - `state`: $[\phi\_{k}, v\_{k}, \delta\_{k}, a\_{k}, w\_{k}]$, since the pose components are zero in $\mathcal{F}\_{k}$ by construction.
    - `obstacles`, `obstacles_mask`: Obstacle polygon vertices expressed in $\mathcal{F}\_{k}$, along with the corresponding boolean mask for padding.
    - `reference`: The local reference expressed in $\mathcal{F}\_{k}$.
    - `direction`: A length-one vector encoding the driving direction (forward or reverse).
4. The action $(u^{v}\_k, u^{\delta}\_k)$ is applied to the vehicle model to obtain the next state $\mathbf{x}\_{k+1}$.
5. The reward $r\_{k}$ is computed from the transition (typically using $\mathbf{x}\_{k}$, $\mathbf{x}\_{k+1}$, and the local reference), and the termination conditions are evaluated (see V.2 for details).
...
Assume that we have $s\_{k}$ denotes the arc-length (i.e., progress) at the current step.

Obviously, it is initialize to 0 at the beginning and updated at every step.

The local reference is constructed by sampling $M$ configurations on the interval $[s\_{k}, s\_{k}^{\mathrm{end}}]$, where the $s\_{k}^{\mathrm{end}}$ is defined using a distance horizon:

```math
\begin{aligned}
v_{ini} &:= |v_k| \\
T_{acc} &:= min(T_{horizon}, \frac{max(0, v_{max} - v_{ini})}{a_{max}}) \\
D_{horizon} &= v_{ini} \cdot T_{acc} + \frac{1}{2} \cdot a_{max} \cdot T_{acc}^{2} + v_{max} \cdot (T_{horizon} - T_{acc}) \\
s_{k}^{\mathrm{end}} &= min(\gamma.\mathrm{length},\, s_k + D_{\mathrm{horizon}})
\end{aligned}
```

The resulting local reference configurations are:
...
```math
e_{track} =
\lambda_x (\frac{x_{k+1} - \overline{x}_{k}}{\sigma_x})^2
+
\lambda_y (\frac{y_{k+1} - \overline{y}_{k}}{\sigma_y})^2
+
\lambda_\psi (\frac{\psi_{k+1} \ominus \overline{\psi}_{k}}{\sigma_\psi})^2
+
\lambda_\phi (\frac{\phi_{k+1} \ominus \overline{\phi}_{k}}{\sigma_\phi})^2
```
...
- $(\overline{x}\_{k}, \overline{y}\_{k}, \overline{\psi}\_{k}, \overline{\phi}\_{k}) = \gamma.\mathbf{q}(s\_{k} + s\_{k}^{lookahead})$ where $s\_{k}^{lookahead} = min(\gamma.\mathrm{length} - s\_{k}, \Delta t \cdot min(v\_{k} + a_{max} \cdot \Delta t, v\_{max}))$
